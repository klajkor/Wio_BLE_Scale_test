...
int prev_state_counter1 = COUNTER_STATE_RESET; //Remembers the previous state (useful to know when the state has changed)
int iSecCounter1 = -1;
...
void StateMachine_counter1(float weight_diff_i, float min_diff_treshold)
{
    ...
    case COUNTER_STATE_COUNTING:
        ...
        if (iSecCounter1 > 0 && weight_diff_i < min_diff_treshold)
        {
